...
plot(F107_case2,'color','k','LineWidth',0.5)
xticks(1:24:120);xticklabels('');xlim([1 120])
ylim([80 100])
ylabel('F10.7','FontSize',ft);%'n_p(cm^{-3})'

h2=subplot(912);%----------------------2 Vp
plot(Vp_case2,'color','k','LineWidth',0.5)
xticks(1:24:120);xticklabels('');xlim([1 120])
ylabel('v_p(km·s^{-1})','FontSize',ft)

h3=subplot(913);%----------------------3 Bz
plot(Bz_case2,'color','k','LineWidth',0.5)
hold on
...
h4=subplot(914);%----------------------4 AE
plot(AE_case2,'color','k','LineWidth',0.5)
xticks(1:24:120);xticklabels('');xlim([1 120])
...
xlim([1 120])
hold on
plot(Dst_case2,'color','k','LineWidth',0.5)
hold off
ylabel('Dst(nT)','FontSize',ft)
x_values = 1:size(Dst_case2, 1); 
y_values_right = JPG_case2; 
yyaxis right;
...
set(gcf,'color','white');
pcolor(lon00,lat00,JRI_tec_case2);
...
shading interp
x_values = 1:size(JRI_tec_case2, 2); 
y_values_right = JPI_tec_case2; 
yyaxis right;
...
h7=subplot(917);%----------------------7 JPT_day
smooth3_density = smoothdata(day_2D_case2,1, 'movmedian',3,'omitnan');  
lat=[88:-2:-88];
[lon00,lat00]=meshgrid(x,lat);
set(gcf,'color','white');
pcolor(lon00,lat00,day_2D_case2);
...
lower_bound = -0.5;
upper_bound = 0.5;
lower_idx = round((lower_bound + 0.5) / 4.5 * (n - 1)) + 1; 
upper_idx = round((upper_bound + 0.5) / 4.5 * (n - 1)) + 1;
new_cmap(lower_idx:upper_idx, :) = repmat([1 1 1], upper_idx - lower_idx + 1, 1);
colormap(gca,new_cmap);   
shading interp;
x_values = 1:size(day_1D_case2, 1); 
y_values_right = day_1D_case2; 
yyaxis right;
plot(x_values, y_values_right, 'm', 'LineWidth', 1.2); 
...
h8=subplot(918);%----------------------8 JPT_night
smooth3_density = smoothdata(night_2D_case2,1, 'movmedian',3,'omitnan');  
lat=[88:-2:-88];
[lon00,lat00]=meshgrid(x,lat);
set(gcf,'color','white');
...
lower_bound = -0.5;
upper_bound = 0.5;
lower_idx = round((lower_bound + 0.5) / 4.5 * (n - 1)) + 1; %8.5
upper_idx = round((upper_bound + 0.5) / 4.5 * (n - 1)) + 1;
new_cmap(lower_idx:upper_idx, :) = repmat([1 1 1], upper_idx - lower_idx + 1, 1);
colormap(gca,new_cmap);
shading interp;
...
x_values = 1:size(night_1D_case2, 1); 
y_values_right = night_1D_case2; 
yyaxis right;
plot(x_values, y_values_right, 'm', 'LineWidth', 1.2); 
...
smooth3_density = smoothdata(day_night_2D,1, 'movmedian',3,'omitnan');  
lat=[88:-2:-88];
[lon00,lat00]=meshgrid(x,lat);
set(gcf,'color','white');
pcolor(lon00,lat00,smooth3_density);
